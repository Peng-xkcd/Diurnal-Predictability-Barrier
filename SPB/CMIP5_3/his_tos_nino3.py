...
for i in range(0, len(list_dir)):
    ...
    if os.path.isfile(path_nc):
        ...
        '''
        Nino 3
                print(np.argwhere(lat == -4.5), np.argwhere(lat == 4.5), np.argwhere(lon == -149.5), \
                      np.argwhere(lon == -90.5)) # NIno3.4区域
                lat_s = np.argwhere(lat == -4.5) #85
                lat_e = np.argwhere(lat == 4.5) #94
                lon_s = np.argwhere(lon == -149.5) #30
                lon_e = np.argwhere(lon == -90.5) #89
        '''
        t_enson1 = np.squeeze(np.mean(np.squeeze(np.mean(sst[:, 85:95, :], axis=1))[:, 30:90], axis=1))
        ...
        plt.title('{} (Niño 3)'.format(list_dir[i]), fontdict={'weight': 'normal', 'size': 20})
        plt.xlabel('Lag(month)', fontdict={'weight': 'normal', 'size': 15})
        plt.ylabel('Initial Month', fontdict={'weight': 'normal', 'size': 15})
        plt.savefig('E:\\JYS_DATA\\Figs-historical_tos_nino3\\{}.png'.format(list_dir[i]))
        plt.close()
